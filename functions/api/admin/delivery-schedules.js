...
export async function onRequestPost(context) {
  ...
  if (action === "save_display") { const values={ product_placement:text(body.product_placement)||"before_summary", show_checkout:body.show_checkout?"1":"0", show_cart:body.show_cart?"1":"0" }; for (const [key,value] of Object.entries(values)) await context.env.DB.prepare("INSERT OR REPLACE INTO delivery_settings (setting_key,setting_value,updated_at) VALUES (?,?,CURRENT_TIMESTAMP)").bind(key,value).run(); return json({success:true}); }
