...
export async function onRequestPost(context) {
  ...
  if (action === "save_display") { const values={ checkout_placement:text(body.checkout_placement)||"after_shipping", checkout_message:text(body.checkout_message), delivery_notification_label:text(body.delivery_notification_label)||"زمان ارسال", delivery_always_visible: text(body.delivery_always_visible) === "0" ? "0" : "1" }; for (const [key,value] of Object.entries(values)) await context.env.DB.prepare("INSERT OR REPLACE INTO delivery_settings (setting_key,setting_value,updated_at) VALUES (?,?,CURRENT_TIMESTAMP)").bind(key,value).run(); return json({success:true}); }
  if (action === "save_payment_location") { const value=JSON.stringify({location:text(body.location),methods:text(body.methods),enabled:!!body.enabled}); await context.env.DB.prepare("INSERT OR REPLACE INTO delivery_settings (setting_key,setting_value,updated_at) VALUES ('payment_location_rule',?,CURRENT_TIMESTAMP)").bind(value).run(); return json({success:true}); }
  if (action === "save_weekly_slots") { const rows = Array.isArray(body.rows) ? body.rows : []; for (const row of rows) { const weekday=num(row.weekday); const title=text(row.title)||"زمان ارسال"; const found = await context.env.DB.prepare("SELECT id FROM delivery_schedules WHERE weekday=? AND specific_date IS NULL AND title=? LIMIT 1").bind(weekday,title).first(); if (found) await context.env.DB.prepare("UPDATE delivery_schedules SET start_time=?,end_time=?,capacity=?,cutoff_minutes=0,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(text(row.start_time),text(row.end_time),num(row.capacity),row.active?1:0,found.id).run(); else if(text(row.start_time) && text(row.end_time)) await context.env.DB.prepare("INSERT INTO delivery_schedules (title,weekday,start_time,end_time,capacity,cutoff_minutes,is_active) VALUES (?,?,?,?,?,0,?)").bind(title,weekday,text(row.start_time),text(row.end_time),num(row.capacity),row.active?1:0).run(); } return json({success:true}); }
  const values=[text(body.title)||"بازه ارسال", body.shipping_method_id ? num(body.shipping_method_id) : null, text(body.province)||null, text(body.city)||null, body.weekday === "" || body.weekday == null ? null : num(body.weekday), text(body.specific_date)||null, text(body.start_time), text(body.end_time), num(body.capacity), body.is_active===false?0:1];
  ...
  if(action === "update") { await context.env.DB.prepare(`UPDATE delivery_schedules SET title=?,shipping_method_id=?,province=?,city=?,weekday=?,specific_date=?,start_time=?,end_time=?,capacity=?,cutoff_minutes=0,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(...values,num(body.id)).run(); }
  else await context.env.DB.prepare(`INSERT INTO delivery_schedules (title,shipping_method_id,province,city,weekday,specific_date,start_time,end_time,capacity,cutoff_minutes,is_active) VALUES (?,?,?,?,?,?,?,?,?,0,?)`).bind(...values).run();
