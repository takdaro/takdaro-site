...
export async function onRequestPost(context) {
  ...
  if (action === "save_defaults") { const values={ minimum_days: String(num(body.minimum_days)), horizon_days: String(Math.max(1,num(body.horizon_days,14))) }; for (const [key,value] of Object.entries(values)) await context.env.DB.prepare("INSERT OR REPLACE INTO delivery_settings (setting_key,setting_value,updated_at) VALUES (?,?,CURRENT_TIMESTAMP)").bind(key,value).run(); return json({success:true}); }
