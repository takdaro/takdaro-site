...
var init_default = __esm({
  "api/account/addresses/[id]/default.js"() {
    init_functionsRoutes_0_41626634850681354();
...
async function onRequestPost10(context) {
  try {
    ...
    const allowedPaymentStatuses = ["pending", "paid", "completed", "failed"];
    if (nextStatus && !allowedOrderStatuses.includes(nextStatus)) {
      return json18({ success: false, error: "invalid_order_status" }, 400);
    }
    if (nextPaymentStatus && !allowedPaymentStatuses.includes(nextPaymentStatus)) {
      return json18({ success: false, error: "invalid_payment_status" }, 400);
    }
      ...
      if (finalPaymentStatus !== oldPaymentStatus) {
        ...
        if (finalPaymentStatus === "paid" || finalPaymentStatus === "completed") {
          ...
          await sendPaymentSuccessNotification2(
            ...
            userData,
            ""
            // روش پرداخت از اینجا مشخص نیست، خالی می‌ماند
          );
        }
...
// ../.wrangler/tmp/pages-FI18T7/functionsRoutes-0.41626634850681354.mjs
var routes;
var init_functionsRoutes_0_41626634850681354 = __esm({
  "../.wrangler/tmp/pages-FI18T7/functionsRoutes-0.41626634850681354.mjs"() {
    init_default();
    init_connect();
    init_disconnect();
