...
var init_password = __esm({
  "lib/password.js"() {
    init_functionsRoutes_0_7476355257027578();
...
async function onRequestPost(context) {
  try {
    ...
    const isAdmin3 = role === "admin" || role === "super_admin";
    if (!isAdmin3) {
      return Response.json(
        {
          success: false,
    ...
    const tokenHash = await hashMobileToken(
      mobileToken
    );
...
async function getMobileUser(context) {
  ...
  const tokenHash = await hashMobileToken2(token);
...
async function requireAdmin(context) {
  const user2 = await getCurrentUser(context);
  if (!user2) {
    return {
      ok: false,
      user: null,
...
// api/v1/admin/rates/current.js
var init_current2 = __esm({
  "api/v1/admin/rates/current.js"() {
    init_functionsRoutes_0_7476355257027578();
    init_current();
  }
...
async function onRequestGet3(context) {
  try {
    ...
    const formattedHistory = history.map((item) => ({
      id: item.id,
      rate: item.rate,
      rate_formatted: `${formatNumber2(item.rate)} \u062A\u0648\u0645\u0627\u0646`,
    ...
    return json2({
      ...
      current_rate: currentRate ? {
        ...
        rate_formatted: `${formatNumber2(currentRate.rate)} \u062A\u0648\u0645\u0627\u0646`,
...
async function onRequestPost3(context) {
  try {
    ...
    await logAdminAction(context, {
      ...
      description: `\u0646\u0631\u062E ${currencyCode} \u0627\u0632 ${formatNumber3(previousRate?.rate || 0)} \u0628\u0647 ${formatNumber3(newRate)} \u062A\u0648\u0645\u0627\u0646 \u062A\u063A\u06CC\u06CC\u0631 \u06CC\u0627\u0641\u062A. (\u0645\u0646\u0628\u0639: ${sourceType})`
    });
    ...
    return json3({
      ...
      rate: {
        ...
        rate_formatted: `${formatNumber3(result.rate?.rate || newRate)} \u062A\u0648\u0645\u0627\u0646`,
        previous_rate: result.previous_rate || null,
        previous_rate_formatted: result.previous_rate ? `${formatNumber3(result.previous_rate)} \u062A\u0648\u0645\u0627\u0646` : null,
...
// api/v1/admin/rates/update-prices.js
var init_update_prices2 = __esm({
  "api/v1/admin/rates/update-prices.js"() {
    init_functionsRoutes_0_7476355257027578();
    init_update_prices();
  }
...
function productFromRow(row, images) {
  ...
  return {
    ...
    display_price_formatted: displayPrice !== null ? `${formatNumber4(displayPrice)} \u062A\u0648\u0645\u0627\u0646` : "\u062A\u0645\u0627\u0633 \u0628\u06AF\u06CC\u0631\u06CC\u062F"
  };
}
...
async function onRequestGet4(context) {
  try {
    ...
    if (!productId) {
      return json5({ success: false, error: "invalid_product_id" }, 400);
    }
    const product = await getProduct(context, productId);
    if (!product) {
      return json5({ success: false, error: "\u0645\u062D\u0635\u0648\u0644 \u067E\u06CC\u062F\u0627 \u0646\u0634\u062F." }, 404);
    }
    return json5({ success: true, product });
  } catch (error) {
    return json5(
      {
        success: false,
...
async function onRequestPut(context) {
  try {
    ...
    const body = await context.request.json().catch(() => null);
    if (!body || typeof body !== "object") {
      return json5({ success: false, error: "invalid_request_body" }, 400);
    }
    const name = cleanText(body.name ?? currentProduct.name, 250);
    const slug = cleanSlug(body.slug ?? currentProduct.slug);
    if (!name) {
      return json5({ success: false, error: "\u0646\u0627\u0645 \u0645\u062D\u0635\u0648\u0644 \u0627\u0644\u0632\u0627\u0645\u06CC \u0627\u0633\u062A." }, 400);
    }
    if (!slug) {
      return json5(
        {
...
async function getCurrentUser2(request, env) {
  const sessionId = getCookie2(request.headers.get("cookie"), "session_id");
...
async function onRequestPost5(context) {
  try {
    const user2 = await getCurrentUser2(context.request, context.env);
    if (!user2) {
      return json6({ success: false, error: "Unauthorized" }, 401);
    }
    const addressId = Number(context.params.id);
    if (!addressId) {
      return json6({ success: false, error: "\u0634\u0646\u0627\u0633\u0647 \u0622\u062F\u0631\u0633 \u0646\u0627\u0645\u0639\u062A\u0628\u0631 \u0627\u0633\u062A." }, 400);
    }
    ...
    if (!existing) {
      return json6({ success: false, error: "\u0622\u062F\u0631\u0633 \u067E\u06CC\u062F\u0627 \u0646\u0634\u062F." }, 404);
    }
    await context.env.DB.prepare(`
      UPDATE addresses
...
async function onRequestPost7(context) {
  try {
    const { request, env } = context;
    const user2 = await getCurrentUser(request, env);
    if (!user2 || !user2.id) {
      return json8(
        {
          success: false,
...
function buildOrderCreatedMessage(orderData, userData, items) {
  ...
  if (Array.isArray(items) && items.length > 0) {
    productsText = items.map((item) => {
      ...
      itemsTotal += total;
      return `  \u2022 ${name} \xD7 ${qty} - ${formatNumber5(total)} \u062A\u0648\u0645\u0627\u0646`;
    }).join("\n");
  } else {
...
`;
  message += `  \u062C\u0645\u0639 \u0645\u062D\u0635\u0648\u0644\u0627\u062A: ${formatNumber5(itemsTotal)} \u062A\u0648\u0645\u0627\u0646
`;
  if (shippingAmount > 0) {
    message += `  \u0647\u0632\u06CC\u0646\u0647 \u0627\u0631\u0633\u0627\u0644: ${formatNumber5(shippingAmount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  if (walletUsedAmount > 0) {
    message += `  \u0628\u0631\u062F\u0627\u0634\u062A \u0627\u0632 \u06A9\u06CC\u0641 \u067E\u0648\u0644: -${formatNumber5(walletUsedAmount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  message += `  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
`;
  message += `  <b>\u0645\u0628\u0644\u063A \u0642\u0627\u0628\u0644 \u067E\u0631\u062F\u0627\u062E\u062A: ${formatNumber5(payableAmount || totalAmount)} \u062A\u0648\u0645\u0627\u0646</b>
`;
  if (cashbackAmount > 0) {
    message += `
  \u{1F381} <b>\u06A9\u0634\u200C\u0628\u06A9 \u0627\u06CC\u0646 \u0633\u0641\u0627\u0631\u0634: ${formatNumber5(cashbackAmount)} \u062A\u0648\u0645\u0627\u0646</b>`;
    message += `
...
`;
  message += `  ${formatNumber5(payableAmount || totalAmount)} \u062A\u0648\u0645\u0627\u0646
...
`;
  message += `  ${formatNumber5(totalAmount)} \u062A\u0648\u0645\u0627\u0646
...
`;
    message += `  ${formatNumber5(refundAmount)} \u062A\u0648\u0645\u0627\u0646
...
`;
  message += `  ${formatNumber5(amount)} \u062A\u0648\u0645\u0627\u0646
...
`;
    message += `  ${formatNumber5(newBalance)} \u062A\u0648\u0645\u0627\u0646

`;
  }
  message += `\u{1F550} <b>\u062A\u0627\u0631\u06CC\u062E \u0648 \u0633\u0627\u0639\u062A:</b>
`;
  message += `  ${formatDate2((/* @__PURE__ */ new Date()).toISOString())}
`;
  return message;
}
...
`;
  message += `  ${formatNumber5(cashbackAmount)} \u062A\u0648\u0645\u0627\u0646
...
`;
  message += `\u{1F550} <b>\u062A\u0627\u0631\u06CC\u062E \u0648 \u0633\u0627\u0639\u062A:</b>
`;
  message += `  ${formatDate2(createdAt || (/* @__PURE__ */ new Date()).toISOString())}
`;
  return message;
}
...
`;
  message += `\u{1F4C5} <b>\u062A\u0627\u0631\u06CC\u062E \u062B\u0628\u062A:</b> ${formatDate2(createdAt)}
...
`;
  message += `\u{1F4B0} <b>\u0645\u0628\u0644\u063A \u06A9\u0627\u0644\u0627\u0647\u0627:</b> ${formatNumber5(itemsTotal)} \u062A\u0648\u0645\u0627\u0646
`;
  if (shippingAmount > 0) {
    message += `\u{1F69A} <b>\u0647\u0632\u06CC\u0646\u0647 \u0627\u0631\u0633\u0627\u0644:</b> ${formatNumber5(shippingAmount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  if (walletUsedAmount > 0) {
    message += `\u{1F45B} <b>\u0628\u0631\u062F\u0627\u0634\u062A \u0627\u0632 \u06A9\u06CC\u0641 \u067E\u0648\u0644:</b> -${formatNumber5(walletUsedAmount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  message += `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
`;
  message += `\u{1F4B3} <b>\u0645\u0628\u0644\u063A \u0646\u0647\u0627\u06CC\u06CC:</b> ${formatNumber5(payableAmount || totalAmount)} \u062A\u0648\u0645\u0627\u0646

`;
  if (cashbackAmount > 0) {
    message += `\u{1F4B0} <b>\u06A9\u0634\u200C\u0628\u06A9 \u0627\u06CC\u0646 \u0633\u0641\u0627\u0631\u0634:</b> ${formatNumber5(cashbackAmount)} \u062A\u0648\u0645\u0627\u0646
`;
...
`;
  ...
  if (cashbackAmount > 0) {
    message += `\u{1F381} <b>\u06A9\u0634\u200C\u0628\u06A9 \u0627\u06CC\u0646 \u0633\u0641\u0627\u0631\u0634:</b> ${formatNumber5(cashbackAmount)} \u062A\u0648\u0645\u0627\u0646
`;
    message += `(\u067E\u0633 \u0627\u0632 \u062A\u06A9\u0645\u06CC\u0644 \u0633\u0641\u0627\u0631\u0634 \u0628\u0647 \u06A9\u06CC\u0641 \u067E\u0648\u0644 \u0634\u0645\u0627 \u0627\u0636\u0627\u0641\u0647 \u0645\u06CC\u200C\u0634\u0648\u062F)
...
`;
  message += `\u{1F4B0} <b>\u0645\u0628\u0644\u063A \u0633\u0641\u0627\u0631\u0634:</b> ${formatNumber5(totalAmount)} \u062A\u0648\u0645\u0627\u0646
...
`;
  ...
  message += `\u{1F550} <b>\u062A\u0627\u0631\u06CC\u062E:</b> ${formatDate2(createdAt)}
...
async function testTelegramNotification(env, _botToken, chatId, userId) {
  const botToken = env.TELEGRAM_BOT_TOKEN;
  if (!botToken) {
    return {
      success: false,
...
async function onRequestPost11(context) {
  try {
    ...
    if (!user_id) {
      return json14({ success: false, error: "user_id required" }, 400);
    }
    if (!password || !password_confirm) {
      return json14({ success: false, error: "password and password_confirm required" }, 400);
    }
    if (password.length < 8) {
      return json14({ success: false, error: "password must be at least 8 characters" }, 400);
    }
    if (password !== password_confirm) {
      return json14({ success: false, error: "password confirmation does not match" }, 400);
    }
    const targetUser = await context.env.DB.prepare("SELECT id, role FROM users WHERE id = ?").bind(user_id).first();
    if (!targetUser) {
      return json14({ success: false, error: "user not found" }, 404);
    }
...
async function onRequestGet7(context) {
  try {
    ...
    if (action === "sms_template") {
      ...
      if (!eventType) {
        return json16({ success: false, error: "eventType \u0627\u0644\u0632\u0627\u0645\u06CC \u0627\u0633\u062A." }, 400);
      }
    ...
    if (action === "sms_template_preview") {
      ...
      const template = await getSmsTemplate(context.env, eventType);
      if (!template) {
        return json16({ success: false, error: "Template \u06CC\u0627\u0641\u062A \u0646\u0634\u062F." }, 404);
      }
      const sampleData = {
        customer_name: "\u06A9\u0627\u0631\u0628\u0631 \u062A\u0633\u062A",
...
async function onRequestPost13(context) {
  try {
    ...
    const body = await context.request.json().catch(() => null);
    if (!body || typeof body !== "object") {
      return json16({ success: false, error: "payload \u0646\u0627\u0645\u0639\u062A\u0628\u0631 \u0627\u0633\u062A." }, 400);
    }
...
async function applyCashbackIfNeeded(db, order, actorUserId) {
  ...
  const cashbackAmount = Math.max(
    0,
    Math.round(normalizeNumber2(order?.cashback_amount))
  );
  ...
  const balanceBefore = Math.max(
    0,
    normalizeNumber2(user2.wallet_balance)
  );
...
async function restoreProductStock(db, orderId) {
  ...
  for (const item of orderItems) {
    ...
    const quantity = Math.max(
      0,
      Math.floor(normalizeNumber2(item?.quantity))
    );
  ...
  for (const [productId, quantity] of quantitiesByProduct) {
    ...
    restored.push({
      ...
      stock_quantity: Math.max(
        0,
        normalizeNumber2(product?.stock_quantity)
      ),
...
async function onRequestGet8(context) {
  try {
    ...
    const user2 = adminAccess.user;
    const url = new URL(context.request.url);
    const search = normalizeText3(
      url.searchParams.get("search")
    );
    const status = normalizeText3(
      url.searchParams.get("status")
    ).toLowerCase();
    const paymentStatus = normalizeText3(
    ...
    const orders = (Array.isArray(result?.results) ? result.results : []).map((order) => {
      ...
      return {
        ...order,
        subtotal_amount: normalizeNumber2(order.subtotal_amount),
        shipping_amount: normalizeNumber2(order.shipping_amount),
        total_amount: normalizeNumber2(order.total_amount),
        wallet_used_amount: normalizeNumber2(order.wallet_used_amount),
        cashback_amount: normalizeNumber2(order.cashback_amount),
        payable_amount: Math.max(
          0,
          normalizeNumber2(
            order.payable_amount != null ? order.payable_amount : normalizeNumber2(order.total_amount) - normalizeNumber2(order.wallet_used_amount)
          )
        ),
        address,
...
async function onRequestPost14(context) {
  try {
    ...
    const nextPaymentStatus = normalizeText3(
      body?.payment_status
    ).toLowerCase();
    if (!orderNumber) {
      return json17(
    ...
    const payableAmount = Math.max(
      0,
      normalizeNumber2(finalOrder.total_amount) - normalizeNumber2(finalOrder.wallet_used_amount)
    );
    try {
      if (finalStatus !== oldStatus) {
        const orderData = {
          ...
          totalAmount: normalizeNumber2(finalOrder.total_amount),
          shippingAmount: normalizeNumber2(finalOrder.shipping_amount),
...
// lib/admin-products.js
function clean(value, max = 1e4) {
...
function adminProductFromRow(row, imagesByProductId, currentRate = null) {
  if (!row) return null;
  ...
  let displayPrice = null;
  if (priceType === "rate_based") {
    if (row.calculated_price !== null && row.calculated_price !== void 0) displayPrice = Number(row.calculated_price);
    else if (Number(row.base_price) > 0 && Number(currentRate) > 0) displayPrice = Number(row.base_price) * Number(currentRate);
  } else if (row.price !== null && row.price !== void 0) displayPrice = Number(row.price);
  return {
    id: productId,
    slug: row.slug || "",
    name: row.name || "",
    ...
    profit_type: row.profit_type || "none",
    profit_value: nullableNumber(row.profit_value),
    fixed_fee: nullableNumber(row.fixed_fee),
    rounding_type: row.rounding_type || "none",
    rounding_method: row.rounding_method || "nearest",
    calculated_price: nullableNumber(row.calculated_price),
    price_calculated_at: row.price_calculated_at || null,
    display_price: displayPrice,
    display_price_formatted: displayPrice !== null ? `${formatNumber6(displayPrice)} \u062A\u0648\u0645\u0627\u0646` : "\u062A\u0645\u0627\u0633 \u0628\u06AF\u06CC\u0631\u06CC\u062F"
  };
}
async function listAdminProducts(env, filters = {}) {
...
var COLUMNS;
var init_admin_products = __esm({
  "lib/admin-products.js"() {
    init_functionsRoutes_0_7476355257027578();
    init_rate();
    COLUMNS = "id, slug, name, category, price, price_label, show_price, stock_quantity, in_stock, stock_label, short_description, description, primary_image, page_url, status, created_at, updated_at, price_type, base_price, profit_type, profit_value, fixed_fee, rounding_type, rounding_method, calculated_price, price_calculated_at";
    __name(clean, "clean");
    __name(idOf, "idOf");
    __name(nullableNumber, "nullableNumber");
    __name(formatNumber6, "formatNumber");
    __name(currentUsdRate, "currentUsdRate");
    __name(getProductImages, "getProductImages");
    __name(adminProductFromRow, "adminProductFromRow");
...
function productFromRow2(row, imagesByProductId, currentRate = null) {
  ...
  return {
    ...
    price_calculated_at: row.price_calculated_at || null,
    display_price: displayPrice,
    display_price_formatted: displayPrice !== null ? `${formatNumber7(displayPrice)} \u062A\u0648\u0645\u0627\u0646` : "\u062A\u0645\u0627\u0633 \u0628\u06AF\u06CC\u0631\u06CC\u062F"
  };
}
...
async function onRequestPost15(context) {
  try {
    ...
    const body = await context.request.json().catch(() => null);
    if (!body || typeof body !== "object") {
      return json18({ success: false, error: "invalid_request_body" }, 400);
    }
...
async function onRequestGet11(context) {
  try {
    ...
    if (action === "costs") {
      ...
      return json20({ success: true, costs });
    }
    if (action === "free-thresholds") {
      ...
      return json20({ success: true, thresholds: enriched });
    }
    return json20({ success: false, error: "invalid_action" }, 400);
  } catch (error) {
    return json20({ success: false, error: String(error?.message || error) }, 500);
  }
}
async function onRequestPost17(context) {
  try {
    ...
    if (action === "create_method") {
      ...
      const default_cost = normalizeNumber3(body.default_cost);
      const is_active = body.is_active === true || body.is_active === "true" ? 1 : 0;
      const sort_order = normalizeNumber3(body.sort_order);
      if (!name || !slug) {
        return json20({ success: false, error: "name_and_slug_required" }, 400);
      }
      const existing = await context.env.DB.prepare(`
        SELECT id FROM shipping_methods WHERE slug = ?
      `).bind(slug).first();
      if (existing) {
        return json20({ success: false, error: "slug_already_exists" }, 400);
      }
      const result = await context.env.DB.prepare(`
    ...
    if (action === "update_method") {
      const id = normalizeNumber3(body.id);
      const name = normalizeText5(body.name);
      const slug = normalizeText5(body.slug).toLowerCase().replace(/\s+/g, "-");
      const description = normalizeText5(body.description);
      const delivery_time = normalizeText5(body.delivery_time);
      ...
      const sort_order = normalizeNumber3(body.sort_order);
      if (!id || !name || !slug) {
        return json20({ success: false, error: "id_name_slug_required" }, 400);
      }
      const existing = await context.env.DB.prepare(`
        SELECT id FROM shipping_methods WHERE id = ?
      `).bind(id).first();
      if (!existing) {
        return json20({ success: false, error: "method_not_found" }, 404);
      }
    ...
    if (action === "delete_method") {
      const id = normalizeNumber3(body.id);
      if (!id) {
        return json20({ success: false, error: "id_required" }, 400);
      }
      const existing = await context.env.DB.prepare(`
        SELECT id FROM shipping_methods WHERE id = ?
      `).bind(id).first();
    ...
    if (action === "delete_cost") {
      const cost_id = normalizeNumber3(body.cost_id);
      if (!cost_id) {
        return json20({ success: false, error: "cost_id_required" }, 400);
      }
      const existing = await context.env.DB.prepare(`
        SELECT id FROM shipping_costs WHERE id = ?
      `).bind(cost_id).first();
      if (!existing) {
        return json20({ success: false, error: "cost_not_found" }, 404);
      }
      await context.env.DB.prepare(`
    ...
    if (action === "save_cost") {
      ...
      const shipping_method_id = normalizeNumber3(body.shipping_method_id);
      const cost_type = body.cost_type || "fixed";
      const cost_amount = normalizeNumber3(body.cost_amount);
      const extra_cost = normalizeNumber3(body.extra_cost);
      const delivery_time = normalizeText5(body.delivery_time);
      const is_active = body.is_active === true || body.is_active === "true" ? 1 : 0;
      if (!province || !city || !shipping_method_id) {
        return json20({ success: false, error: "province_city_method_required" }, 400);
      }
    ...
    if (action === "add_city") {
      ...
      `).first();
      if (!defaultMethod) {
        return json20({ success: false, error: "default_method_not_found" }, 404);
      }
      const existing = await context.env.DB.prepare(`
        SELECT id FROM shipping_costs 
      ...
      if (existing) {
        return json20({ success: false, error: "city_already_exists" }, 400);
      }
      const result = await context.env.DB.prepare(`
        INSERT INTO shipping_costs (province, city, shipping_method_id, cost_type, cost_amount, extra_cost, delivery_time, is_active)
        VALUES (?, ?, ?, 'extra', 0, 0, '', 1)
    ...
    if (action === "save_free_threshold") {
      const shipping_method_id = normalizeNumber3(body.shipping_method_id);
      const min_order_amount = normalizeNumber3(body.min_order_amount);
      const is_active = body.is_active === true || body.is_active === "true" ? 1 : 0;
      if (!shipping_method_id || !min_order_amount) {
        return json20({ success: false, error: "method_and_amount_required" }, 400);
      }
      const method = await context.env.DB.prepare(`
        SELECT id FROM shipping_methods WHERE id = ?
      `).bind(shipping_method_id).first();
  ...
  try {
    ...
    if (userId > 0) {
      const user2 = await db.prepare(`
        ...
        `).bind(userId).first();
      if (!user2) {
        return json21({ success: false, error: "user_not_found" }, 404);
      }
...
async function onRequestPost18(context) {
  try {
    ...
    if (!userId || amount <= 0) {
      return json21({ success: false, error: "user_id_and_amount_required" }, 400);
    }
    if (!type) {
      return json21({ success: false, error: "invalid_type" }, 400);
    }
    const user2 = await db.prepare(`
        SELECT
          id,
    ...
    if (balanceAfter < 0) {
      return json21({ success: false, error: "insufficient_wallet_balance" }, 400);
    }
    await db.batch([
      db.prepare(`
        UPDATE users
...
async function getShippingCost(db, province, city, subtotal = 0) {
  const normalizedProvince = normalizeText7(province);
  const normalizedCity = normalizeText7(city);
  ...
  if (freeThreshold && subtotal >= normalizeNumber4(freeThreshold.min_order_amount)) {
    finalCost = 0;
    isFree = true;
  }
  return {
    ...
    shipping: {
      ...
      free_threshold: freeThreshold ? normalizeNumber4(freeThreshold.min_order_amount) : null
    }
  };
}
async function onRequestPost19(context) {
  try {
    const body = await context.request.json().catch(() => null);
    if (!body) {
      return json22({ success: false, error: "invalid_payload" }, 400);
    }
    const province = normalizeText7(body.province);
    const city = normalizeText7(body.city);
    const subtotal = normalizeNumber4(body.subtotal);
    if (!province || !city) {
      return json22({ success: false, error: "province_and_city_required" }, 400);
    }
    const result = await getShippingCost(context.env.DB, province, city, subtotal);
    if (!result.success) {
      return json22({ success: false, error: result.message }, 404);
    }
    return json22({
      success: true,
...
// api/v1/shipping/calculate.js
var init_calculate2 = __esm({
  "api/v1/shipping/calculate.js"() {
    init_functionsRoutes_0_7476355257027578();
    init_calculate();
  }
...
// api/v1/shipping/provinces.js
var init_provinces2 = __esm({
  "api/v1/shipping/provinces.js"() {
    init_functionsRoutes_0_7476355257027578();
    init_provinces();
  }
...
async function onRequestPut4(context) {
  try {
    ...
    if (!user2) {
      return json25({ success: false, error: "Unauthorized" }, 401);
    }
    const addressId = Number(context.params.id);
    if (!addressId) {
      return json25({ success: false, error: "\u0634\u0646\u0627\u0633\u0647 \u0622\u062F\u0631\u0633 \u0646\u0627\u0645\u0639\u062A\u0628\u0631 \u0627\u0633\u062A." }, 400);
    }
    const existing = await getOwnedAddress(context.env, user2.id, addressId);
    if (!existing) {
      return json25({ success: false, error: "\u0622\u062F\u0631\u0633 \u067E\u06CC\u062F\u0627 \u0646\u0634\u062F." }, 404);
    }
    const body = await context.request.json();
    const data = normalizeAddressInput(body);
    const validationError = validateAddressInput(data);
    if (validationError) {
      return json25({ success: false, error: validationError }, 400);
    }
...
async function onRequestGet15(context) {
  try {
    ...
    if (!orderNumber) {
      return json26({ success: false, error: "order_number_required" }, 400);
    }
    ...
    if (!order) {
      return json26({ success: false, error: "order_not_found" }, 404);
    }
...
async function restoreProductStock2(db, orderId) {
  const items = await getOrderItems2(db, orderId);
  if (!items.length) {
    return {
      success: true,
      restored: []
  ...
  for (const item of items) {
    ...
    const quantity = Math.max(
      0,
      Math.round(
        normalizeNumber5(item?.quantity)
      )
    );
    if (!productId || quantity <= 0) {
      continue;
  ...
  for (const [productId, quantity] of quantities) {
    ...
    restored.push({
      ...
      stock_quantity: Math.max(
        0,
        normalizeNumber5(product?.stock_quantity)
      ),
      in_stock: Number(product?.in_stock) === 1
    });
  }
...
async function applyCashbackIfNeeded2(db, order, actorUserId) {
  ...
  const cashbackAmount = Math.max(
    0,
    Math.round(
      normalizeNumber5(order?.cashback_amount)
    )
  );
  if (!orderId || !userId || cashbackAmount <= 0) {
    await db.prepare(`
  ...
  const balanceBefore = Math.max(
    0,
    normalizeNumber5(user2.wallet_balance)
  );
  const balanceAfter = balanceBefore + cashbackAmount;
  await db.batch([
    db.prepare(`
...
async function onRequestPost20(context) {
  try {
    ...
    const body = await context.request.json().catch(() => null);
    const orderNumber = normalizeText8(
      context.params?.order || body?.order_number || ""
    );
    const nextStatus = normalizeText8(
      body?.status
    ).toLowerCase();
    if (!orderNumber) {
      return json27(
    ...
    try {
      ...
      const orderData = {
        orderId: finalOrder.id,
        orderNumber: finalOrder.order_number,
        totalAmount: normalizeNumber5(
          finalOrder.total_amount
        ),
        shippingAmount: normalizeNumber5(
          finalOrder.shipping_amount
        ),
        walletUsedAmount: normalizeNumber5(
          finalOrder.wallet_used_amount
        ),
        payableAmount,
        cashbackAmount: normalizeNumber5(
          finalOrder.cashback_amount
        ),
        status: finalStatus,
        paymentStatus: finalPaymentStatus,
    ...
    return json27({
      ...
      order: {
        ...finalOrder,
        subtotal_amount: normalizeNumber5(
          finalOrder.subtotal_amount
        ),
        shipping_amount: normalizeNumber5(
          finalOrder.shipping_amount
        ),
        total_amount: normalizeNumber5(
          finalOrder.total_amount
        ),
        wallet_used_amount: normalizeNumber5(
          finalOrder.wallet_used_amount
        ),
        cashback_amount: normalizeNumber5(
          finalOrder.cashback_amount
        ),
        payable_amount: payableAmount,
        items
...
async function onRequestPost21(context) {
  try {
    ...
    const orderNumber = normalizeText9(
      context.params?.order || body?.order_number || ""
    );
    const nextStatus = normalizeText9(
      body?.status
    ).toLowerCase();
    if (!orderNumber) {
      return json28(
    ...
    let stockRestoreResult = null;
    if (oldStatus !== "cancelled" && finalStatus === "cancelled") {
      stockRestoreResult = await restoreProductStock3(
        context.env.DB,
        currentOrder.id
      );
      if (!stockRestoreResult.success) {
    ...
    try {
      ...
      const orderData = {
        orderId: finalOrder.id,
        orderNumber: finalOrder.order_number,
        totalAmount: normalizeNumber6(
          finalOrder.total_amount
        ),
        shippingAmount: normalizeNumber6(
          finalOrder.shipping_amount
        ),
        walletUsedAmount: normalizeNumber6(
          finalOrder.wallet_used_amount
        ),
        payableAmount,
        cashbackAmount: normalizeNumber6(
          finalOrder.cashback_amount
        ),
        status: finalStatus,
        paymentStatus: finalPaymentStatus,
...
function normalizeStatus3(value, fallback = "draft") {
  const status = cleanText3(
    value,
    30
  ).toLowerCase();
  if ([
...
function normalizeImages3(value, fallbackImages = []) {
...
async function getProductImages3(db, productId) {
  const result = await db.prepare(`
        SELECT
          id,
          image_url,
...
async function getProductPayload2(db, productId, currentRate = null) {
  const row = await db.prepare(`
        SELECT
          id,
          slug,
  ...
  const images = await getProductImages3(
    db,
    productId
  );
  let displayPrice = null;
...
async function onRequestGet18(context) {
  try {
    ...
    const payload = await getProductPayload2(
      context.env.DB,
      product.id,
      currentRate
    );
...
async function onRequestPut5(context) {
  try {
    ...
    const currentImages = await getProductImages3(
      context.env.DB,
      currentProduct.id
    );
    const name = body.name !== void 0 ? cleanText3(
      body.name,
      250
    ) : String(
      currentProduct.name || ""
    );
    const slug = body.slug !== void 0 ? cleanText3(
      body.slug,
      160
    ).toLowerCase() : String(
      currentProduct.slug || ""
    );
    const category = body.category !== void 0 ? cleanText3(
      body.category,
      120
    ) : String(
    ...
    const price = body.price !== void 0 ? toOptionalPrice3(
      body.price
    ) : currentProduct.price === null || currentProduct.price === void 0 ? null : Number(
      currentProduct.price
    );
    const priceLabel = body.price_label !== void 0 || body.priceLabel !== void 0 ? cleanText3(
      body.price_label ?? body.priceLabel,
      100
    ) || "\u062A\u0645\u0627\u0633 \u0628\u06AF\u06CC\u0631\u06CC\u062F" : String(
      currentProduct.price_label || "\u062A\u0645\u0627\u0633 \u0628\u06AF\u06CC\u0631\u06CC\u062F"
    );
    const showPrice = body.show_price !== void 0 || body.showPrice !== void 0 ? toBooleanInteger3(
      body.show_price ?? body.showPrice,
      price !== null
    ) : Number(
    ...
    const inStock = body.in_stock !== void 0 || body.inStock !== void 0 ? toBooleanInteger3(
      body.in_stock ?? body.inStock,
      stockQuantity > 0
    ) : Number(
      currentProduct.in_stock
    ) === 1;
    const stockLabel = body.stock_label !== void 0 || body.stockLabel !== void 0 ? cleanText3(
      body.stock_label ?? body.stockLabel,
      100
    ) || (inStock ? "\u0645\u0648\u062C\u0648\u062F" : "\u0646\u0627\u0645\u0648\u062C\u0648\u062F") : String(
      currentProduct.stock_label || (inStock ? "\u0645\u0648\u062C\u0648\u062F" : "\u0646\u0627\u0645\u0648\u062C\u0648\u062F")
    );
    const shortDescription = body.short_description !== void 0 || body.shortDescription !== void 0 ? cleanText3(
      body.short_description ?? body.shortDescription,
      1e3
    ) : String(
      currentProduct.short_description || ""
    );
    const description = body.description !== void 0 ? cleanText3(
      body.description,
      2e4
    ) : String(
      currentProduct.description || ""
    );
    const pageUrl = body.page_url !== void 0 || body.pageUrl !== void 0 ? cleanText3(
      body.page_url ?? body.pageUrl,
      500
    ) : String(
      currentProduct.page_url || ""
    );
    const status = body.status !== void 0 ? normalizeStatus3(
      body.status,
      String(
    ...
    const basePrice = body.base_price !== void 0 || body.basePrice !== void 0 ? toOptionalPrice3(
    ...
    const profitType = body.profit_type !== void 0 || body.profitType !== void 0 ? cleanText3(
      body.profit_type ?? body.profitType,
      30
    ) : String(
      currentProduct.profit_type || "none"
    );
    const profitValue = body.profit_value !== void 0 || body.profitValue !== void 0 ? toOptionalPrice3(
    ...
    const fixedFee = body.fixed_fee !== void 0 || body.fixedFee !== void 0 ? toOptionalPrice3(
    ...
    const roundingType = body.rounding_type !== void 0 || body.roundingType !== void 0 ? cleanText3(
      body.rounding_type ?? body.roundingType,
      30
    ) : String(
      currentProduct.rounding_type || "none"
    );
    const roundingMethod = body.rounding_method !== void 0 || body.roundingMethod !== void 0 ? cleanText3(
    ...
    const images = body.images !== void 0 ? normalizeImages3(
      body.images
    ) : normalizeImages3(
      currentImages
    );
    const primaryImage = body.primary_image !== void 0 || body.primaryImage !== void 0 ? normalizeImageUrl3(
      body.primary_image ?? body.primaryImage
    ...
    const product = await getProductPayload2(
      context.env.DB,
      currentProduct.id,
      currentRate
    );
...
async function onRequestGet19(context) {
  try {
    const user2 = await getCurrentUser(context);
    if (!user2) return json30({ success: false, error: "Unauthorized" }, 401);
    const result = await context.env.DB.prepare(`
    ...
    `).bind(user2.id).all();
    return json30({ success: true, addresses: result.results || [] });
  } catch (error) {
    return json30({ success: false, error: String(error?.message || error) }, 500);
  }
}
async function onRequestPost22(context) {
  try {
    ...
    if (validationError) {
      return json30({ success: false, error: validationError }, 400);
    }
    if (data.is_default === 1) {
      await context.env.DB.prepare(`
        UPDATE addresses
    ...
    return json30({ success: true, id: insertedId, address });
  } catch (error) {
    return json30({ success: false, error: String(error?.message || error) }, 500);
  }
}
async function onRequestPut6(context) {
  try {
    ...
    if (!id) {
      return json30({ success: false, error: "\u0622\u062F\u0631\u0633 \u0645\u0648\u0631\u062F \u0646\u0638\u0631 \u06CC\u0627\u0641\u062A \u0646\u0634\u062F." }, 400);
    }
    const body = await context.request.json();
    const data = normalizeAddressInput2(body);
    const validationError = validateAddressInput2(data);
    ...
    if (!existing) {
      return json30({ success: false, error: "\u0622\u062F\u0631\u0633 \u0645\u0648\u0631\u062F \u0646\u0638\u0631 \u06CC\u0627\u0641\u062A \u0646\u0634\u062F." }, 404);
    }
    if (data.is_default === 1) {
      await context.env.DB.prepare(`
        UPDATE addresses
    ...
    const updated = await context.env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(id, user2.id).first();
    return json30({ success: true, address: updated });
  } catch (error) {
    return json30({ success: false, error: String(error?.message || error) }, 500);
  }
}
async function onRequestDelete7(context) {
  try {
    ...
    return json30({ success: true, message: "\u0622\u062F\u0631\u0633 \u0628\u0627 \u0645\u0648\u0641\u0642\u06CC\u062A \u062D\u0630\u0641 \u0634\u062F." });
  } catch (error) {
    return json30({ success: false, error: String(error?.message || error) }, 500);
  }
}
...
function normalizeOrderNumber(value) {
  const result = normalizeText10(value);
  return result || null;
}
...
async function onRequestPost23(context) {
  try {
    ...
    const currentStatus = normalizeText10(
      order.status
    ).toLowerCase();
    ...
    if (changes !== 1) {
      ...
      if (normalizeText10(latestOrder?.status).toLowerCase() === "cancelled") {
        return json31({
          success: true,
          already_cancelled: true,
          order: {
    ...
    const stockRestoreResult = await restoreProductStock4(
      context.env.DB,
      order.id
    );
    return json31({
...
function extractItemUnitPrice(item) {
  if (item?.displayPrice !== void 0 && item?.displayPrice !== null) {
    const displayPrice = normalizeNumber7(item.displayPrice);
    if (displayPrice > 0) {
      return displayPrice;
    }
  }
  const directPrice = normalizeNumber7(item?.unit_price);
  if (directPrice > 0) {
    return directPrice;
  }
  const price = normalizeNumber7(item?.price);
  if (price > 0) {
    return price;
  }
  const productPrice = normalizeNumber7(item?.product?.price);
  if (productPrice > 0) {
    return productPrice;
  }
  const rowTotal = normalizeNumber7(
...
function extractItemTotalPrice(item) {
  const directTotal = normalizeNumber7(
...
async function getCashbackSettings(db) {
  ...
  try {
    ...
    const cashbackStatuses = normalizeStatuses2(
...
async function createOrUpdateAddress(context, user2, address) {
  const fullName = normalizeText11(address.full_name) || normalizeText11(user2.full_name);
  const addressLine = normalizeText11(address.address_line);
  ...
  const city = normalizeText11(address.city);
  const state = normalizeText11(address.state);
...
async function onRequestPost24(context) {
  try {
    ...
    const order = body.order || {};
    const items = Array.isArray(order.items) ? order.items : [];
    const shippingAmount = normalizeNumber7(
      order.shipping_amount
    );
    const submittedSubtotalAmount = normalizeNumber7(
      order.subtotal_amount
    );
    const submittedTotalAmount = normalizeNumber7(
      order.total_amount
    );
    ...
    const requestedWalletUse = normalizeNumber7(
      order.wallet_used_amount ?? order.wallet_amount ?? body.wallet_used_amount
    );
    const balanceBefore = normalizeNumber7(
      user2.wallet_balance
    );
    ...
    const orderInsert = await context.env.DB.prepare(`
      ...
      normalizeText11(order.notes || body.notes)
    ).run();
...
async function onRequestGet20(context) {
  try {
    const user2 = await getCurrentUser(context);
    if (!user2) {
      return json33({ success: false, error: "unauthorized" }, 401);
    }
...
function normalizeStatuses3(rawValue) {
...
async function getWalletSettings(env) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      return {
        cashback_percent: cashbackPercent,
        cashback_statuses: normalizeStatuses3(map.cashback_statuses)
      };
    } catch (error) {
...
async function onRequestGet21(context) {
  try {
    ...
    if (!user2) {
      return json34({ success: false, error: "unauthorized" }, 401);
    }
...
async function onRequestGet24(context) {
  try {
    ...
    if (!adminCheck.ok) return adminCheck.response;
    const url = new URL(context.request.url);
    const search = normalizeText12(url.searchParams.get("search"));
    const role = normalizeText12(url.searchParams.get("role"));
...
async function onRequestPost25(context) {
  try {
    ...
    const body = await context.request.json();
    const user_id = Number(body.user_id || 0);
    const full_name = normalizeText12(body.full_name);
    const email = normalizeEmail2(body.email);
    const phone = normalizePhone3(body.phone);
    const role = normalizeText12(body.role || "user");
...
async function onRequestGet26(context) {
  try {
    const user2 = await getCurrentUser(context);
    if (!user2) {
      return json35({ success: false, error: "unauthorized" }, 401);
    }
    return json35({
      success: true,
      user: {
    ...
    const orders = rows.map(
      (order) => {
        ...
        return {
          ...
          status: order.status || "payment_pending",
          payment_status: order.payment_status || "pending",
          subtotal_amount: normalizeNumber8(
            order.subtotal_amount
          ),
          shipping_amount: normalizeNumber8(
            order.shipping_amount
          ),
          total_amount: normalizeNumber8(
            order.total_amount
          ),
          wallet_used_amount: normalizeNumber8(
            order.wallet_used_amount
          ),
          cashback_amount: normalizeNumber8(
            order.cashback_amount
          ),
          payable_amount: Math.max(
            0,
...
async function onRequestGet29(context) {
  try {
    ...
    return json37({ success: true, ...result });
  } catch (error) {
    console.error("Mobile products error:", error);
    return json37({ success: false, error: String(error?.message || error) }, 500);
  }
}
...
`;
  message += `\u{1F4C5} <b>\u062A\u0627\u0631\u06CC\u062E \u062B\u0628\u062A:</b>
`;
  message += `  ${formatDate2(order.created_at)}
...
`;
    for (const item of items) {
      const total = Number(item.total_price || 0);
      const qty = Number(item.quantity || 0);
      message += `  \u2022 ${item.product_name || "\u0645\u062D\u0635\u0648\u0644"} \xD7 ${qty} - ${formatNumber5(total)} \u062A\u0648\u0645\u0627\u0646
`;
    }
    message += `
`;
...
`;
  message += `  \u062C\u0645\u0639 \u0645\u062D\u0635\u0648\u0644\u0627\u062A: ${formatNumber5(order.subtotal_amount || 0)} \u062A\u0648\u0645\u0627\u0646
`;
  if (order.shipping_amount > 0) {
    message += `  \u0647\u0632\u06CC\u0646\u0647 \u0627\u0631\u0633\u0627\u0644: ${formatNumber5(order.shipping_amount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  if (order.wallet_used_amount > 0) {
    message += `  \u0628\u0631\u062F\u0627\u0634\u062A \u0627\u0632 \u06A9\u06CC\u0641 \u067E\u0648\u0644: -${formatNumber5(order.wallet_used_amount)} \u062A\u0648\u0645\u0627\u0646
`;
  }
  message += `  \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
`;
  message += `  <b>\u0645\u0628\u0644\u063A \u0642\u0627\u0628\u0644 \u067E\u0631\u062F\u0627\u062E\u062A: ${formatNumber5(order.payable_amount || order.total_amount)} \u062A\u0648\u0645\u0627\u0646</b>
`;
  if (order.cashback_amount > 0) {
    message += `
  \u{1F381} <b>\u06A9\u0634\u200C\u0628\u06A9 \u0627\u06CC\u0646 \u0633\u0641\u0627\u0631\u0634: ${formatNumber5(order.cashback_amount)} \u062A\u0648\u0645\u0627\u0646</b>`;
  }
...
function productFromRow3(product, imageRows, rate) {
  ...
  if (priceType === "rate_based" && basePrice !== null && basePrice > 0 && rate !== null) {
    displayPrice = calculatedPrice !== null && calculatedPrice > 0 ? calculatedPrice : calculateProductPrice(product, rate);
  } else if (priceType === "fixed") {
    const price = Number(product.price);
...
async function getPublishedProducts(env, filters = {}) {
  const db = env?.DB;
  if (!db) {
    throw new Error("D1 database binding DB is not configured.");
  }
  const requestedSlug = cleanText4(filters.slug);
  const requestedCategory = cleanText4(filters.category);
  ...
  const productsQuery = "SELECT p.id, p.slug, p.name, p.category, p.price, p.price_label, p.show_price, p.stock_quantity, p.in_stock, p.stock_label, p.short_description, p.description, p.primary_image, p.page_url, p.status, p.created_at, p.updated_at, p.price_type, p.base_price, p.profit_type, p.profit_value, p.fixed_fee, p.rounding_type, p.rounding_method, p.calculated_price, p.price_calculated_at FROM products p WHERE " + conditions.join(" AND ") + " ORDER BY CASE WHEN p.in_stock = 1 AND p.stock_quantity > 0 THEN 0 ELSE 1 END, p.id ASC";
  ...
  const imagesQuery = "SELECT id, product_id, image_url, alt_text, sort_order, is_primary FROM product_images WHERE product_id IN (" + placeholders + ") ORDER BY product_id ASC, is_primary DESC, sort_order ASC, id ASC";
...
var init_products4 = __esm({
  "lib/products.js"() {
    ...
    __name(getPublishedProducts, "getPublishedProducts");
  }
});

// api/products.js
function json39(data, status = 200, headers = {}) {
  return Response.json(data, {
    status,
    headers: {
      "Cache-Control": "public, max-age=60, s-maxage=60",
...
async function onRequestGet31(context) {
  try {
    ...
    const result = await getPublishedProducts(context.env, {
      slug: cleanText5(url.searchParams.get("slug")),
      category: cleanText5(url.searchParams.get("category")),
      includeOutOfStock: url.searchParams.get("include_out_of_stock") === "1"
    });
    return json39({ success: true, ...result });
  } catch (error) {
    return json39(
      { success: false, error: String(error?.message || error) },
      500,
...
// ../.wrangler/tmp/pages-bICOwF/functionsRoutes-0.7476355257027578.mjs
var routes;
var init_functionsRoutes_0_7476355257027578 = __esm({
  "../.wrangler/tmp/pages-bICOwF/functionsRoutes-0.7476355257027578.mjs"() {
    init_login2();
    init_logout2();
    init_me2();
