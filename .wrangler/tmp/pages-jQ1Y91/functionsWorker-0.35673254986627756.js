...
var init_default = __esm({
  "api/account/addresses/[id]/default.js"() {
    init_functionsRoutes_0_06720013250228751();
...
// api/products.js
function json26(data, status = 200, headers = {}) {
  return Response.json(data, {
    status,
    headers: {
      "Cache-Control": "public, max-age=60, s-maxage=60",
...
async function onRequestGet22(context) {
  try {
    const db = context.env?.DB;
    if (!db) {
      return json26({ success: false, error: "D1 database binding DB is not configured." }, 500);
    }
    const url = new URL(context.request.url);
    ...
    if (!productRows.length) {
      return json26({ success: true, total: 0, products: [], rate: currentRate });
    }
    ...
    return json26({ success: true, total: products.length, products, rate: currentRate });
  } catch (error) {
    return json26({ success: false, error: String(error?.message || error) }, 500, { "Cache-Control": "no-store" });
  }
}
...
// ../.wrangler/tmp/pages-jQ1Y91/functionsRoutes-0.06720013250228751.mjs
var routes;
var init_functionsRoutes_0_06720013250228751 = __esm({
  "../.wrangler/tmp/pages-jQ1Y91/functionsRoutes-0.06720013250228751.mjs"() {
    init_default();
    init_password2();
    init_id();
