...
async function insertOrderRecord(db, payload) {
  try {
    ...
    const message = String(error?.message || error || "");
    const ignorable = message.includes("has no column named payable_amount") || message.includes("table orders has no column named payable_amount") || message.includes("no such column: payable_amount");
...
__name(insertOrderRecord, "insertOrderRecord");
async function onRequestPost5(context) {
  try {
    ...
    const orderInsert = await insertOrderRecord(context.env.DB, {
      ...
      notes: normalizeText2(order.notes || body.notes)
    });
...
async function onRequestGet11(context) {
  try {
    ...
    if (!adminCheck.ok) return adminCheck.response;
    const url = new URL(context.request.url);
    const search = normalizeText4(url.searchParams.get("search"));
    const role = normalizeText4(url.searchParams.get("role"));
...
async function onRequestPost8(context) {
  try {
    ...
    const body = await context.request.json();
    const user_id = Number(body.user_id || 0);
    const full_name = normalizeText4(body.full_name);
    const email = normalizeEmail(body.email);
    const phone = normalizePhone3(body.phone);
    const role = normalizeText4(body.role || "user");
...
function normalizeWalletType(value) {
  const type = normalizeText5(value).toLowerCase();
...
function normalizeStatuses3(input) {
  ...
  const normalized = list.map((item) => normalizeText5(item).toLowerCase()).filter(Boolean);
  ...
  try {
    ...
    if (userId > 0) {
      const user = await db.prepare(`
        ...
        `).bind(userId).first();
      if (!user) {
        return json12({ success: false, error: "user_not_found" }, 404);
      }
...
async function onRequestPost9(context) {
  try {
    ...
    await ensureWalletTables(db);
    const body = await context.request.json().catch(() => null);
    const action = normalizeText5(body?.action).toLowerCase();
    ...
    const note = normalizeText5(pickFirst(body?.note, body?.description));
    const source = normalizeText5(
      pickFirst(body?.source, body?.reference_type, body?.referenceType, "admin")
    ).toLowerCase() || "admin";
    const referenceType = normalizeText5(
      pickFirst(body?.reference_type, body?.referenceType, source, "admin")
    ).toLowerCase() || "admin";
    const referenceId = normalizeText5(
    ...
    const orderNumber = normalizeText5(
      pickFirst(body?.order_number, body?.orderNumber)
    );
    if (!userId || amount <= 0) {
      return json12({ success: false, error: "user_id_and_amount_required" }, 400);
    }
    if (!type) {
      return json12({ success: false, error: "invalid_type" }, 400);
    }
    const user = await db.prepare(`
        SELECT
          id,
    ...
    if (balanceAfter < 0) {
      return json12({ success: false, error: "insufficient_wallet_balance" }, 400);
    }
    await db.batch([
      db.prepare(`
        UPDATE users
...
// api/products.js
function json14(data, status = 200, headers = {}) {
  return Response.json(data, {
    status,
    headers: {
...
// ../.wrangler/tmp/pages-4m7yq8/functionsRoutes-0.00026067275459717987.mjs
var routes = [
  {
